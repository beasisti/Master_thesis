...
# Select stations 2007_2023  -----------------------------------------------------

ozono <- read_csv('./Dati_iniziali/O3_completed.csv')

# I take the stations with observations across this whole period
data_apr_1_2007 <- ozono %>%
  filter(timestamp >= as.POSIXct("2007-04-01 00:00:00") & timestamp < as.POSIXct("2007-04-02 00:00:00"))
stations_apr_1_2007 <- data_apr_1_2007 %>%
  filter(!is.na(Stato)) %>%
  pull(idSensore) %>%
  unique()
stations_apr_1_2007 <- c(stations_apr_1_2007, c(10270, 10282, 10288)) # from the NA_image

final_ozono <- ozono %>%
  filter(idSensore %in% stations_apr_1_2007)
# these are all the stations for which we have data in 2007-2023

final_ozono_2007 <- subset(final_ozono, timestamp >= as.POSIXct("2007-04-01 00:00:00"))


rm(ozono, data_apr_1_2007, final_ozono)


# Map of stations 2007_2023  -----------------------------------------------------
...
X <- X %>%
  filter(Station %in% stations_apr_1_2007)
...
# 180 model  ------------------------------------------------------------------------------------
## Dealing with NAs 2007_2023  -----------------------------------------------------

massimi <- final_ozono_2007 %>%
...
# write.csv(count_180_df, "./Datasets_2007_23/Dataset_180.csv", row.names = FALSE)

keep(stations_apr_1_2007, final_ozono_2007, sure = TRUE)


## Plot NAs 2007_2023  -----------------------------------------------------

count_180_df <- read.csv("./Datasets_2007_23/Dataset_180.csv")
...
sum(is.na(count_180_df$Count_180)) / nrow(count_180_df) # 0.0833872
...
ozono_120 <- ozono_120 %>%
  filter(idSensore %in% stations_apr_1_2007)
ozono_120_2007 <- subset(ozono_120, timestamp >= as.POSIXct("2007-03-31 00:00:00"))
...
# write.csv(count_120_df, "./Datasets_2007_23/Dataset_120.csv", row.names = FALSE)

keep(stations_apr_1_2007, sure = TRUE)


## Plot NAs 2007_2023  -----------------------------------------------------

count_120_df <- read.csv("./Datasets_2007_23/Dataset_120.csv")
...
sum(is.na(count_120_df$Count_120)) / nrow(count_120_df) # 0.06679595
...
X_2007 <- subset(X, Year >= 2007 & Year <= 2023)
...
colonne_da_standardizzare <- c("mean_temperature", "mean_precipitation_sum", 
                               ...
                               "Density", "Quota")
X_2007[, colonne_da_standardizzare] <- scale(X_2007[, colonne_da_standardizzare])
...
# dummy for Zone
baseline <- "C" # mountain
X_2007$Zone <- as.factor(X_2007$Zone)
X_2007$Zone <- factor(X_2007$Zone, levels = c(baseline, setdiff(levels(X_2007$Zone), baseline)))
zone_dummies <- model.matrix(~ Zone - 1, data = X_2007)
zone_dummies <- zone_dummies[, -1]
colnames(zone_dummies) <- paste("Zone", levels(X_2007$Zone)[-1], sep = "_")
X_final <- cbind(X_2007, zone_dummies)
X_final <- X_final[, !names(X_final) %in% "Zone"]

# dummy for Area
baseline <- "R" # rural
X_2007$Area <- as.factor(X_2007$Area) 
X_2007$Area <- factor(X_2007$Area, levels = c(baseline, setdiff(levels(X_2007$Area), baseline))) 
area_dummies <- model.matrix(~ Area - 1, data = X_2007)
area_dummies <- area_dummies[, -1]
colnames(area_dummies) <- paste("Area", levels(X_2007$Area)[-1], sep = "_")
X_final <- cbind(X_final, area_dummies)
X_final <- X_final[, !names(X_final) %in% "Area"]

# dummy for Type
baseline <- "B" # background
X_2007$Type <- as.factor(X_2007$Type) 
X_2007$Type <- factor(X_2007$Type, levels = c(baseline, setdiff(levels(X_2007$Type), baseline))) 
type_dummies <- model.matrix(~ Type - 1, data = X_2007)
type_dummies <- type_dummies[, -1]
colnames(type_dummies) <- paste("Type", levels(X_2007$Type)[-1], sep = "_")
X_final <- cbind(X_final, type_dummies)
X_final <- X_final[, !names(X_final) %in% "Type"]


# write.csv(X_final, "./Datasets_2007_23/variables_to_select.csv", row.names = FALSE)
...
# write.csv(dist_mat, "./Datasets_2007_23/distances.csv")
